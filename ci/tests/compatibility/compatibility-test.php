...
class Compatibility_Test {
	...
	public function activate_sut_and_dependencies() {
		...
		foreach ( $this->suts as $sut_slug => $sut_path ) {
			echo PHP_EOL . "Installing and Activating SUT: $sut_slug\n";
			// WooCommerce is not the SUT.
			if ( ! in_array( $this->main_sut_slug, [ 'woocommerce', 'wporg-woocommerce' ] ) ) {
				echo $this->exec_in_container("mv {$this->sut_dir}/$sut_slug /var/www/html/wp-content/{$this->main_sut_type}s/$sut_slug") . PHP_EOL;

				$this->exec_in_container_and_log_stderr( sprintf( 'wp %s activate "%s"', $this->main_sut_type, str_replace( '"', '', $sut_slug ) ) );
			} else {
