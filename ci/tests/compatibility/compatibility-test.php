...
$compatibility->activate_woocommerce();
$compatibility->activate_sut_and_dependencies();
...
class Compatibility_Test {
	...
	protected function __construct() {
		...
		$this->sut_dir                   = getenv( 'GITHUB_WORKSPACE' ) . '/ci/plugins';
		...
		$before_removing_suts = count( $this->extensions_to_test );

		$this->extensions_to_test = array_diff_key( $this->extensions_to_test, $this->suts );

		// Check that we removed the SUTs as expected.
		if ( $before_removing_suts - count( $this->extensions_to_test ) !== count( $this->suts ) ) {
			echo sprintf( 'Expected to have removed %d SUT plugins from list, but have removed %d.', count( $this->suts ), $before_removing_suts - count( $this->extensions_to_test ) );
			die( 1 );
		}

		require_once __DIR__ . '/../../bin/ci-runner.php';
	...
	public function exec_in_container( string $command, bool $stop_if_error = true, array $allowed_exit_codes = [ 0 ], array $force_stop_if_error_code = [] ) {
		exec( sprintf( 'docker exec --tty --user=www-data ci_runner_php_fpm bash -c "%s"', addcslashes( $command, '"' ) ), $output, $exit_status );
	...
	public function activate_woocommerce() {
		// Activate WooCommerce, unless WooCommerce is the SUT.
		if ( ! in_array( $this->main_sut_slug, [ 'woocommerce', 'wporg-woocommerce' ] ) ) {
			echo $this->exec_in_container( 'wp plugin activate woocommerce' ) . PHP_EOL;
		}
	}

	public function activate_sut_and_dependencies() {
		...
		$this->activating_sut = true;

		foreach ( $this->suts as $sut_slug => $sut_path ) {
			echo "Installing and Activating SUT: $sut_slug\n";
			if ( ! in_array( $this->main_sut_slug, [ 'woocommerce', 'wporg-woocommerce' ] ) ) {
				// WooCommerce is not the SUT.
				$this->exec_in_container( sprintf( 'wp plugin install "%s"', str_replace( '"', '', $sut_path ) ) );
				$this->exec_in_container_and_log_stderr( sprintf( 'wp plugin activate "%s"', str_replace( '"', '', $sut_slug ) ) );
			} else {
				// WooCommerce is the SUT.
				$this->exec_in_container_and_log_stderr( 'wp plugin activate woocommerce' );
			}
		...
		foreach ( $this->extensions_to_test as $plugin_slug => $plugin_dir_path ) {
			...
			if ( $plugin_slug !== 'activation_test' ) {
				...
				if ( strpos( $plugin_dir_path, '.zip' ) === false ) {
					// Already extracted. On Compatibility Tests we pre-extract all zips for performance.
					$link  = "ln -s $plugin_dir_path /var/www/html/wp-content/plugins/$plugin_slug || echo '/var/www/html/wp-content/plugins/$plugin_slug already exists'";
					$start = microtime( true );
					$this->exec_in_container( "$link" );
					$timings['Link'] = $benchmark( $start );
				} else {
					// Zipped plugin.
					$start = microtime( true );
					echo 'Executing install...' . PHP_EOL;
					$this->exec_in_container( sprintf( "wp plugin install \"%s\"", addcslashes( $plugin_dir_path, '"' ) ) );
					$timings['Plugin Install'] = $benchmark( $start );
				}
			...
			if ( $activated ) {
				...
				echo "Skipped visiting pages because the plugin failed to activate.\n";
			}
