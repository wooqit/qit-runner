...
test.describe( 'Cart & Checkout Restricted Coupons', () => {
	...
	test.skip( 'coupon cannot be used by any customer (email restricted)', async ( {
		page,
	} ) => {
		await page.goto( `/shop/?add-to-cart=${ firstProductId }` );
		await page.waitForLoadState( 'networkidle' );
	...
	test.skip( 'coupon can be used by the right customer (email restricted) but only once', async ( {
		page,
		baseURL,
	} ) => {
		const api = new wcApi( {
