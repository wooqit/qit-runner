...
test.describe.serial( 'WooCommerce Orders > Refund an order', () => {
	...
	test.skip( 'can issue a refund by quantity', async ( { page } ) => {
	...
	test.skip( 'can delete an issued refund', async ( { page } ) => {
		await page.goto( `wp-admin/post.php?post=${ orderId }&action=edit` );
...
test.describe( 'WooCommerce Orders > Refund and restock an order item', () => {
	...
	test.skip( 'can update order after refunding item without automatic stock adjustment', async ( {
		page,
	} ) => {
		await page.goto( `wp-admin/post.php?post=${ orderId }&action=edit` );
